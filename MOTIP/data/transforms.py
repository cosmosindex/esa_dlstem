...
class MultiColorJitter:
    ...
    def __call__(self, images, annotations, metas):
        if isinstance(images, torch.Tensor):
            ...
            assert isinstance(images[0], Image.Image)
            params = _v2_params(self.color_jitter, [images[0]])
            images = [_v2_apply(self.color_jitter, _, params) for _ in images]
        else:
            raise NotImplementedError(f"The input image type {type(images)} is not supported.")
        return images, annotations, metas


def _v2_params(transform, imgs):
    """torchvision renamed v2 transforms' `_get_params` to `make_params` (0.20+).

    MOTIP targets 0.19; this repo runs 0.25. Call whichever exists so the same
    code works on both instead of pinning torchvision for one method.
    """
    fn = getattr(transform, "_get_params", None) or getattr(transform, "make_params")
    return fn(imgs)


def _v2_apply(transform, img, params):
    """Companion to _v2_params: `_transform` became `transform` in the same move."""
    fn = getattr(transform, "_transform", None) or getattr(transform, "transform")
...
class MultiRandomPhotometricDistort:
    ...
    def __call__(self, images, annotations, metas):
        _params = _v2_params(self.ramdom_photometric_distort, [images[0]])
        images = [_v2_apply(self.ramdom_photometric_distort, _, _params) for _ in images]
        return images, annotations, metas
