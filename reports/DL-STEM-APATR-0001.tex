...
Tables~\ref{tab:mot-per-category} and~\ref{tab:mot-per-size} present the MOT evaluation results on the BIRDSAI test set. 
Several observations can be drawn from these results.

\textbf{Identity consistency.}A striking finding is that SAM2 produces only 506 ID switches across the entire test set, 
compared to 10\,686 for Faster R-CNN and 9\,064 for YOLO. This is a direct consequence of SAM2's prompt-based propagation mechanism: 
each object is initialised once and tracked through a learned correspondence model, avoiding the repeated detection-to-track association that 
ByteTracker must perform for the other two methods. While this yields superior identity consistency, it also means that SAM2 cannot recover objects 
that were missed in the initial prompt frame or that disappear and reappear later.

\textbf{Per-category analysis.}
