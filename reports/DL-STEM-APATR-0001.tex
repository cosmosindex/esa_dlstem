...
\tocentry{Datasets and Experimental Setup}{2.\enspace Datasets and Experimental Setup}{4}
\tocsub{dataset-preprocessing}{2.1.\enspace Dataset Preprocessing}{4}
\tocsub{dataset-preprocessing}{\hspace{1.2em}2.1.1.\enspace Data Augmentation}{5}
\tocsub{Datasets and Experimental Setup}{2.2.\enspace Experimental Setup}{5}
\tocentry{algorithms-assessment}{3.\enspace Algorithms Assessment}{6}
\tocsub{Overall performance}{3.1.\enspace Overall Performance}{6}
\tocsub{OP_SOT}{\hspace{1.2em}3.1.1.\enspace Single Object Tracking (SOT)}{6}
\tocsub{OP_MOT}{\hspace{1.2em}3.1.2.\enspace Multi-Object Tracking (MOT)}{7}
\tocsub{algorithms-assessment}{3.2.\enspace Performance at different Testing Dimensions}{9}
\tocsub{Performance by Object Size}{3.3.\enspace Performance by Object Size}{10}
\tocsub{SOT}{\hspace{1.2em}3.3.1.\enspace Single Object Tracking (SOT)}{10}
\tocsub{MOT}{\hspace{1.2em}3.3.2.\enspace Multi-Object Tracking (MOT)}{15}
\tocsub{algorithms-assessment}{3.4.\enspace Performance by Frame per Second (FPS)}{20}
\tocsub{algorithms-assessment}{3.5.\enspace Performance by Signal to Noise Ratio (SNR)}{20}
\tocsub{algorithms-assessment}{3.6.\enspace Performance by Motion Blur and Occlusion}{20}
...
The objective is to assess and compare selected algorithms for object detection and tracking in EO videos, examining their performance on the datasets specified in 
Test Specification D5 (Section 2.2), across both Single Object Tracking (SOT) and Multi-Object Tracking (MOT) tasks under varying data conditions.

\subsection{Document Structure}
The report is structured as follows. Section 2 describes the datasets and experimental setup, covering the data preprocessing pipeline, augmentation strategies, 
and the hyperparameter configurations used for each algorithm. Section 3 presents the algorithm assessment results. It begins with an overall performance summary 
for both SOT and MOT tasks on the BIRDSAI and OOTB datasets, followed by a compliance overview across all test dimensions. Section 3 then provides a detailed 
breakdown of performance by object size and category, combining quantitative metrics with qualitative visualisations to illustrate key failure modes and model behaviours. 
Results for Grounding DINO and evaluation across remaining test dimensions are pending and will be reported in version 2 of this document.

\hypertarget{Datasets and Experimental Setup}{%
\section{Datasets and Experimental Setup}\label{Experimental Setup}}

In this report, we present results on two datasets specified in Test Specification D5 
(Section 2.2), OOTB and BIRDSAI, one from Use Case 2: vehicle detection and tracking 
and one from Use Case 1: wild animal detection and tracking. 
...
The preprocessing pipeline consists of two main steps: splitting each dataset into training, validation, and test subsets, and applying data 
augmentation during training to improve model generalisation. Regarding the data split, we have mentioned in detail in the test specification 
D5 document how the dataset tested in this report is split.
...
The horizontal flip simulates the natural left--right symmetry of aerial and satellite observations, increasing the diversity of object orientations seen by the model 
without introducing unrealistic artefacts.

\textbf{Evaluation transform.} During validation and testing, only a deterministic resize to $640 \times 640$ pixels is applied, with no stochastic operations, ensuring 
reproducible evaluation metrics across runs.
...
All experiments use AdamW as the optimiser with a weight decay of $10^{-4}$. 
The learning rate follows a cosine schedule with a linear warmup over 5 epochs, 
and training runs for a maximum of 50 epochs with early stopping (patience = 10, 
monitored on \texttt{val/AP50}, the definition of which can be found in Test 
Specification D5 Section 4.2). Mixed precision (16-bit) is used throughout to reduce 
memory usage and accelerate training, and all runs are logged to Weights \& Biases, 
an experiment tracking platform.

\textbf{Faster R-CNN v2} uses a ResNet-50 FPN v2 backbone pretrained on COCO~\cite{lin2014microsoft}. 
The last 2 layers of the backbone are set as trainable, while the remaining layers are frozen, 
and the RPN head and ROI box predictor are fine-tuned, 
...
The predicted boxes from the last frame of each clip are automatically used as prompts for the first frame of the 
next clip, allowing tracking to continue across the full sequence without any additional manual input.

\textbf{DINOv3} (grounding DINO) will be updated in version 2.
...
Its implementation is currently ongoing, and results will be reported in the report version 2.

In the Test Specification D5, we also introduced the Single Object Tracking (SOT) and Multi Object Tracking (MOT) tasks respectively. 
The reason for discussing them as two separate tasks is that the datasets mentioned in the Test Specification D5 (Section 2.2) vary in 
applicability — some are only suitable for the SOT task, while others are applicable to both SOT and MOT tasks. 
This is related to the type of annotations provided by each dataset.

\hypertarget{Overall performance}{%
\subsection{Overall performance}\label{Overall performance}}
We briefly cross-referenced the performance figures reported in the original papers against the performance assessment benchmarks documented 
in Test Specification D5 4.3, and consider the numerical results obtained on our test datasets to be consistent and reasonable.
...
Table~\ref{tab:sot-overall} summarises the overall SOT performance of SAM2 on the 
BIRDSAI dataset and OOTB dataset.

\paragraph{BIRDSAI.}

On the BIRDSAI dataset, SAM2 achieves a Success AUC of 0.3661 and Precision@20 of 0.8881. 
The high Precision@20 indicates that the predicted bounding box centre is within 20 
pixels of the ground truth for the majority of frames, but the moderate Success AUC 
reveals that the overlap between predicted and ground-truth boxes is often insufficient.

\paragraph{OOTB.}
On the OOTB traffic dataset, SAM2 achieves an overall Success AUC of 0.3099, a Precision@20 of 0.5038, and a Mean IoU of 0.2880 over 3\,200 frames.
...
its overall MOTA of $-1.16$ is the worst among all three methods. 

In contrast, YOLO11n strikes a better balance between precision (0.4635) and recall (0.4595), 
yielding the best overall MOTA ($-0.20$) and IDF1 (0.4615). 

SAM2, operating in a zero-shot setting with ground-truth bounding-box prompts on the first frame only, achieves moderate precision (0.4116) 
but low recall (0.2211), as it can only propagate objects that are visible in the prompted frame and tends to lose tracks as objects undergo appearance changes or temporary occlusions.
...
\subsection{Performance at different Testing Dimensions}

Apart from the general performance, the selected algorithms are evaluated across 
multiple testing dimensions as defined in Test Specification D5 (Section 2.1), including 
object size (with a particular focus on small objects), frame rate and video duration, 
signal-to-noise ratio (SNR), and general image quality (motion blur and occlusion).

A preliminary overview of the method compliance w.r.t. different test dimensions 
is summarised in Table~\ref{tab:compliance-overview}, and will be completed in 
version 2 of this document. It provides a high-level compliance overview for each tested algorithm across all test dimensions defined in D5.
...
Detailed performance results across each test dimension are presented in the 
...
Tables~\ref{tab:sot-birdsai} present the per-category and per-size SOT evaluation results for SAM2 on the BIRDSAI test sets. As shown in Table~\ref{tab:sot-birdsai}, SAM2 tracks animals more accurately (Success AUC = 0.4295, Mean IoU = 0.4238) than humans (Success AUC = 0.3347, Mean IoU = 0.3235).
...
\subsection{Performance by Motion Blur and Occlusion}
