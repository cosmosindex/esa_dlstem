...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, transform):
        ...
        add_mot_folder(args.mot_train_name)
    ...
    def _pre_single_frame(self, vid, idx: int):
        ...
        # dtype=long is REQUIRED: satellite frames can have zero GT objects, and
        # torch.as_tensor([]) defaults to float32 — an empty float `labels` then
        # promotes a torch.cat of per-frame labels to float, breaking the
        # matcher's `cost[:, tgt_ids]` indexing (DanceTrack never has empty frames).
