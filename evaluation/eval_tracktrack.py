"""
Run TrackTrack over cached HiEUM detections + FastReID features and score
against GT.

Mirrors ``eval_tracker.py`` but consumes the richer feature cache produced
by ``cache_hieum_dets_with_feats.py`` (one ``.npz`` per video) and uses
the TrackTrack-specific ``[N, 6+D]`` per-frame layout:

    columns: x1, y1, x2, y2, score, _pad, *feat_D

``dets_95`` is currently the same set as ``dets`` — HiEUM's Soft-NMS
produces a single decay-scored pool, so the upstream "looser-NMS rescue"
path is a no-op. Refine later by re-running HiEUM with two NMS settings
if rescue ever becomes worth wiring up.
...
Outputs to ``EXPERIMENT_DIR``::
    ...
    mot_format/<video>.txt     MOTChallenge-format output
...
from models.trackers import TrackTrackTracker
...
def _load_feat_cache(cache_dir: Path, video_id: str) -> dict:
    """Returns dict with `frame_ids`, `boxes_per_frame`, `scores_per_frame`,
    `feats_per_frame` (each list aligned with `frame_ids`)."""
    ...
    boxes = z["boxes"]
    scores = z["scores"]
    feats = z["feats"].astype(np.float32)  # promote to fp32 for downstream math
...
# ----------------------------------------------------------------------
# Per-frame matching (centroid OR IoU, mirrors eval_tracker.py)
# ----------------------------------------------------------------------
...
def main():
    ...
    tracker_name  = cfg.get("tracker", "tracktrack")
    ...
    if tracker_name.lower() != "tracktrack":
        raise ValueError(
    ...
    print("=" * 60)
    print(f"TrackTrack eval on {dataset_key}")
    ...
    tracker = TrackTrackTracker(**tracker_cfg)
    ...
    for v_idx, video in enumerate(dataset.videos, 1):
        ...
        for fid in video.frame_ids:
            ...
            # Build [N, 6+D] for upstream Tracker.update.
            if len(boxes):
                pad = np.zeros((len(boxes), 1), dtype=np.float32)  # column 5: unused by Track init
            ...
            # dets_95 — same set; rescue path is a no-op.
            tracks = tracker.update_with_feats(dets_arr, dets_arr)
