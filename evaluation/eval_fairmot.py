"""Evaluate a trained FairMOT (one-shot JDT) model on a car MOT test split.

FairMOT does its OWN detection + ReID every frame (it cannot consume the
cached HiEUM detections the TBD trackers share), so this script runs the
FairMOT ``JDETracker`` frame-by-frame over each test sequence and scores the
resulting (box, id) tracks against GT — exactly the output contract of
``eval_tracktrack.py`` so the downstream HOTA / master-CSV merge is unchanged:
...
Predictions use the dataset's RAW frame_id; compute_hota.py applies the
1-indexing offset when it builds the TrackEval workspace, so GT and preds stay
aligned. NO visualization is produced (records-only, per request).

The model is a single UNION model — run this once per test split with the same
``--checkpoint`` to get the three per-dataset rows.

Inference preprocessing replicates FairMOT's own ``LoadImages`` (letterbox ->
RGB CHW -> /255), the exact path the model was trained/tested under upstream.
The pedestrian ``min_box_area`` / ``vertical`` (w/h>1.6) filters in upstream
``track.py`` are NOT applied — they would delete tiny / wide satellite cars.
...
# resolved before we juggle sys.path for FairMOT's (colliding) top-level names.
...
_FAIRMOT_LIB = str(_REPO / "FairMOT" / "src" / "lib")

# FairMOT's src/lib exposes top-level packages (models, tracker, tracking_utils,
# utils, opts, lib) that collide with this repo's. We import our datasets above,
# then purge those names and prepend FairMOT's lib so JDETracker's internal
# absolute imports (`from models.model import ...`) bind to FairMOT's copies.
_FAIRMOT_COLLIDING = {"models", "tracker", "tracking_utils", "utils", "opts", "lib"}


def _activate_fairmot_lib():
    for key in [k for k in list(sys.modules)
                if k.split(".")[0] in _FAIRMOT_COLLIDING]:
        del sys.modules[key]
    # Drop the repo root from sys.path: our repo's `models/` is a regular
    # package (has __init__.py) and would shadow FairMOT/src/lib/models even
    # when the latter is prepended (`import models.decode` -> our models ->
    # ModuleNotFoundError). Our dataset classes are already imported (held by
    # reference), so we no longer need the repo root on the path.
    _repo = str(_REPO)
    sys.path[:] = [p for p in sys.path if p not in ("", ".", _repo)]
    if _FAIRMOT_LIB in sys.path:
        sys.path.remove(_FAIRMOT_LIB)
    sys.path.insert(0, _FAIRMOT_LIB)
...
# `_SAM3_CLASS_MAPS`). The integer values are irrelevant downstream (TrackEval
# pools every track as one foreground class); only "present vs dropped" matters.
_ALLCLASS_MAPS = {
    "rscardata":   {"car": 0},
...
def _letterbox(img, height, width, color=(127.5, 127.5, 127.5)):
    """FairMOT's letterbox (datasets/dataset/jde.py), inlined to avoid importing
    FairMOT's `datasets` package. Channel-order agnostic (pad colour symmetric)."""
...
def _preprocess(frame_rgb, input_h, input_w, device):
    """Replicate FairMOT LoadImages: letterbox -> RGB CHW -> /255 (no mean/std).
    `frame_rgb` is HxWx3 uint8 RGB (dataset._load_frame). Training feeds RGB/255,
    so this matches. Returns (im_blob[1,3,H,W], frame_rgb) — img0 used for shape."""
    lb = _letterbox(frame_rgb, input_h, input_w)                # RGB letterboxed
    img = np.ascontiguousarray(lb.transpose(2, 0, 1), dtype=np.float32) / 255.0
    im_blob = torch.from_numpy(img).unsqueeze(0).to(device)
    return im_blob, frame_rgb


# ----------------------------------------------------------------- GT + matching
# (matching helpers mirror eval_tracktrack.py exactly so both scripts score
...
def main():
    ...
    if all_class:                                   # input size is per-dataset bucket
        ...
        input_w = int(cfg.get("input_w", 1920))
        input_h = int(cfg.get("input_h", 1088))
    ...
    print("=" * 60)
    print(f"FairMOT eval on {dataset_key}  (test split, records-only)")
    print(f"ckpt:   {checkpoint}")
    print(f"input:  {input_w}x{input_h}  conf_thres={conf_thres}  K={K}")
    ...
    # ---- build the FairMOT tracker (after our datasets are imported) ----
    _activate_fairmot_lib()
    ...
    if all_class and not gt_oracle:
        # Class-agnostic pooled tracking: the 4-class model emits car/plane/
        # ship/train peaks; stock update() keeps only class-1 (car) via
        # merge_outputs(...)[1]. We override post_process to inverse-affine ALL
        # K decoded boxes (order preserved → id_feature stays aligned) and pool
        # them under key 1, so every class flows into one association pool —
        # exactly the single-foreground protocol compute_hota scores against.
        from utils.image import transform_preds  # noqa: E402  (FairMOT's copy)
    ...
    for v_idx, video in enumerate(dataset.videos, 1):
        ...
        for fid in video.frame_ids:
            ...
            im_blob, img0 = _preprocess(frame_rgb, input_h, input_w, device)
            if gt_oracle:
                online = tracker.update_oracle(im_blob, img0, gt[fid]["boxes"])
            else:
                online = tracker.update(im_blob, img0)
            n_frames_total += 1
    ...
    summary = {
        "tracker": "fairmot_oracle" if gt_oracle else ("fairmot_all" if all_class else "fairmot"),
