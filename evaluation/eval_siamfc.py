"""
Unified SiamFC (ECCV 2016 workshops) evaluation script.

SiamFC is the original fully-convolutional Siamese tracker: AlexNet embedding
plus a cross-correlation response map, no proposals and no online update.
Runs frame-by-frame with a cached exemplar, produces HBB outputs. Uses the
huanglianghua/siamfc-pytorch `TrackerSiamFC` via our thin wrapper.
...
def main():
    parser = argparse.ArgumentParser(description="SiamFC evaluation")
    ...
    variant = cfg.get("variant", "alexnet_e50")
    prompt_strategy = cfg.get("prompt_strategy", "first_frame")
    prompt_interval = cfg.get("prompt_interval", 10)

    run_name = f"siamfc_{variant}_{prompt_strategy}_{dataset_name.lower()}"
    if prompt_strategy == "every_n":
        run_name = f"siamfc_{variant}_every{prompt_interval}_{dataset_name.lower()}"
    ...
    tracker = SiamFCTracker(ckpt_path=ckpt_path)
    ...
    print("=" * 60)
    print(f"SiamFC Evaluation: {variant} | {prompt_strategy} | {dataset_name} | "
