"""2-panel synchronized qualitative video for BIRDSAI SAM3 — DETECTION + TRACKING.

Combined ("二合一") counterpart to viz_birdsai_sam3_panel.py (detection-only),
mirroring the detector combined view in viz_birdsai_track_panel.py but with the
SAM3 GT | SAM3 two-panel layout.

The SAM3 panel shows BOTH at once:
  ...
  * tracking / identity — a trailing line (centre path over the last TRAIL_LEN
    frames) coloured by track id, plus the id number. Continuous same-colour path
    = one identity kept; the id number changing = an ID switch.

The id-trail palette avoids green / red / orange so the two colour schemes never
collide. Tracks are score-filtered at --score (SAM3 emits many low-confidence
boxes; default 0.2 matches the detection-only SAM3 panel).

Usage:
    python evaluation/viz_birdsai_sam3_track_panel.py --video 0000000371_0000000000 \
        --mode xexemplar --score 0.2 --iou 0.3 --fps 12 [--max-frames 800] [--gif]
"""
import argparse
import json
import sys
...
MODE_ANN = {"exemplar_detect": "annotations", "gt_init": "annotations",
            "xexemplar": "annotations_sam3"}

# identity palette — blues / cyans / magentas / purples / yellows ONLY, never
# green / red / orange, so it can't be confused with TP / FP / FN.
_PALETTE = [
...
def load_video_tracks(vid, pred_path):
    """frame_id(str) -> tracks dict {boxes,scores,labels,track_ids}."""
    d = json.load(open(pred_path))
    if vid not in d["videos"]:
        return {}
    return {f: e.get("tracks", {"boxes": [], "scores": [], "labels": [], "track_ids": []})
...
def main():
    ...
    ap.add_argument("--mode", choices=["exemplar_detect", "gt_init", "xexemplar"], default="xexemplar")
    ap.add_argument("--pred", default=None, help="override predictions.json path")
    ap.add_argument("--annotations", default=None, help="GT subdir (default: per-mode)")
    ap.add_argument("--score", type=float, default=0.2, help="keep tracks with score>=this")
    ap.add_argument("--iou", type=float, default=0.3, help="TP match IoU")
    ...
    mp4_path = out_dir / f"{vid}_sam3_{args.mode}_track_panel.mp4"

    rgb0 = ds._load_frame(video, frame_ids[0]); H, W = rgb0.shape[:2]
    canvas_w, canvas_h = W * 2, (H + HEADER_H) + 24
    vw = cv2.VideoWriter(str(mp4_path), cv2.VideoWriter_fourcc(*"mp4v"),
                         args.fps, (canvas_w, canvas_h))
    gif_frames = []

    hist = {k: defaultdict(lambda: deque(maxlen=TRAIL_LEN)) for k in ["GT", "SAM3"]}
    ...
    for fid in frame_ids:
        ...
        t = tracks.get(str(fid), {"boxes": [], "scores": [], "labels": [], "track_ids": []})
        tb = np.asarray(t["boxes"], np.float32).reshape(-1, 4)
        tsc = np.asarray(t.get("scores", []), np.float32).reshape(-1)
        tl = np.asarray(t.get("labels", []), np.int64).reshape(-1)
        tid = np.asarray(t.get("track_ids", []), np.int64).reshape(-1)
        if len(tsc) == len(tb) and len(tb):
            keep = tsc >= args.score
            tb, tl, tid = tb[keep], tl[keep], tid[keep]

        gt_panel = _gt_panel(rgb, gtb, gt_ns, hist["GT"])
        sam3_panel = _model_panel(rgb, gtb, gtl, tb, tl, tid, title, hist["SAM3"], args.iou)

        grid = np.vstack([np.hstack([gt_panel, sam3_panel]), _legend(canvas_w)])
        vw.write(grid)
        if args.gif:
    ...
    if args.gif and gif_frames:
        try:
            import imageio
            gif_path = out_dir / f"{vid}_sam3_{args.mode}_track_panel.gif"
