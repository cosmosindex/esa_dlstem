"""
Report the actual OOTB split produced by `OOTBDataset` for docs.
...
Runs the dataset loader for each of the three splits, then prints:
  ...
  2. Attribute positive counts per split (for `docs/ootb_split_attributes.md`).
"""
import sys
...
from datasets.ootb import ATTR_NAMES, OOTBDataset  # noqa: E402

ROOT = Path("/data/ESA_DLSTEM_2025/data/trafic/OOTB")
RATIOS = (0.8, 0.1, 0.1)
...
def main() -> None:
    ...
    for split in SPLITS:
        ds = OOTBDataset(root=ROOT, split=split, mode="video", clip_len=1)
